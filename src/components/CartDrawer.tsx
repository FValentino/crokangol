...
function buildWhatsAppMessage(items: CartItem[], total: number): string {
  const lines = items.map(
    (i, idx) =>
      `${idx + 1}. ${i.name} — ${i.price} x ${i.quantity} = ${formatPrice(i.priceValue * i.quantity)}`,
  )
  return (
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-pastel">
          ...
          <button onClick={() => setOpen(false)} className="text-dark/50 hover:text-dark text-2xl leading-none" aria-label="Cerrar">
            ✕
          </button>
        </div>
          ...
          <>
            ...
            <div className="border-t border-pastel px-6 py-5 space-y-3">
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 w-full bg-primary text-white font-semibold py-3 rounded-full hover:scale-[1.02] transition-transform shadow-lg shadow-primary/30"
              >
              ...
              <button
                onClick={clearCart}
                className="w-full text-center font-body text-sm text-gray hover:text-primary transition-colors"
              >
                Vaciar carrito
              </button>
            </div>
